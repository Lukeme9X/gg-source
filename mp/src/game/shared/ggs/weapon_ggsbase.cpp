...
IMPLEMENT_NETWORKCLASS_ALIASED(WeaponGGSBase, DT_WeaponGGSBase)

BEGIN_NETWORK_TABLE(CWeaponGGSBase, DT_WeaponGGSBase)
#ifdef CLIENT_DLL
RecvPropInt(RECVINFO(m_iBurstCurr)),
RecvPropBool(RECVINFO(m_bInZoom)),
RecvPropTime(RECVINFO(m_flAttackEnds)),
RecvPropInt(RECVINFO(m_iStance)),
#else
SendPropInt(SENDINFO(m_iBurstCurr)),
...
BEGIN_PREDICTION_DATA(CWeaponGGSBase)
END_PREDICTION_DATA()

//LINK_ENTITY_TO_CLASS(weapon_GGSBase, CWeaponGGSBase);
//PRECACHE_WEAPON_REGISTER(weapon_GGSBase);

acttable_t CWeaponGGSBase::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponGGSBase);



//modify this part to control the general accuracy of the gun

const Vector& CWeaponGGSBase::GetBulletSpread(void)
{
	...
	if (m_iBurstCurr != BURST)
	{
		for (int i = m_iBurstCurr; i < BURST; i++)
		{
			cone += VECTOR_CONE_1DEGREES;
		}
	}
	...
	return *new Vector( cone );
}

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponGGSBase::CWeaponGGSBase(void)
{
	BURST = GetWpnData().m_iBurst;
	ROF = GetWpnData().m_flCycleTime;
	m_iBurstCurr = BURST;
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::Precache(void)
{
	BaseClass::Precache();
}
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::DryFire(void)
{
	WeaponSound(EMPTY);
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::PrimaryAttack(void)
{
	//Hackey, fix asap
	if (first)
	{
	...
	Msg("ROF: %f\n", ROF);
	Msg("BURST: %i\n", BURST);
	Msg("GetWpnData().m_iBurst: %i\n", GetWpnData().m_iBurst);
	Msg("GetWpnData().m_flCycleTime: %f\n", GetWpnData().m_flCycleTime);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponGGSBase::ItemPreFrame(void)
{
	GetStance();
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponGGSBase::ItemBusyFrame(void)
{
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::ItemPostFrame(void)
{
...
// Output : int
//-----------------------------------------------------------------------------
Activity CWeaponGGSBase::GetPrimaryAttackActivity(void)
{
	if (m_iBurstCurr != 0)
	{
...
//-----------------------------------------------------------------------------
bool CWeaponGGSBase::Reload(void)
{
...
//----------------------------------------------------------------------------- 
bool CWeaponGGSBase::Holster(CBaseCombatWeapon *pSwitchingTo /* = NULL */)
{
	if (m_bInZoom)
	{
		ToggleZoom();
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::AddViewKick(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	if (pPlayer == NULL)
	{
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::ToggleZoom(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
...
//-----------------------------------------------------------------------------
void CWeaponGGSBase::CheckZoomToggle(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
...
//----------------------------------------------------------------------------- 
void CWeaponGGSBase::GetStance(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	if (pPlayer == NULL)
	{
